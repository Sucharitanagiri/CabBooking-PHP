                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php if(isset($row['status']) && $row['status'] == 1): ?>
                                    <button class="btn btn-flat btn-warning border btn-sm make_payment"  data-id="<?= $row['id'] ?>" type="button" id="make_payment">Make Payment</button>
                                <?php endif; ?>
                                <button type="button" class="btn btn-flat btn-info border btn-sm view_data" data-id="<?= $row['id'] ?>">View Details</button>

                                <?php if(isset($row['status']) && $row['status'] == 4 && $row['is_review_added'] == 0): ?>
                                    <button type="button" class="btn btn-flat btn-warning border btn-sm give_rating" data-id="<?= $row['id'] ?>">Give Rating</button>
                                <?php endif; ?>
                            </td>
...
<script>
    $(function(){
        ...
        $('.make_payment').click(function(){
            uni_modal("Make Payment","make_payment.php?id="+$(this).attr('data-id'))
        })
        $('.give_rating').click(function(){
            uni_modal("Give Rating","give_rating.php?id="+$(this).attr('data-id'))
        })
    })
</script>